...
namespace HospitalTest.ScheduleConsiliumTests
{
    public class ScheduleConsiliumLogicTests
    {
        ...
        public async Task Schedule_Consilium_Success()
        {
            ...
            var mockService = new Mock<IDoctorService>();
            var mockUnitOfWork = new Mock<IUnitOfWork>();
            var mockConsiliumRepo = new Mock<IConsiliumRepository>();
            var mockRoomRepo = new Mock<IRoomRepository>();
            var mockDoctor = new Mock<Doctor>();
            mockUnitOfWork.Setup(uw => uw.ConsiliumRepository).Returns(mockConsiliumRepo.Object);
            mockUnitOfWork.Setup(uw => uw.RoomRepository).Returns(mockRoomRepo.Object);
            mockUnitOfWork.Setup(uw => uw.ConsiliumRepository
                    .CreateAsync(consilium))
                .ReturnsAsync(() => consilium);
            mockService.Setup(d => d.GetDoctorsForConsilium(consilium))
                .ReturnsAsync(() => doctorList);
            mockDoctor.Setup(d => d.IsAvailable(consilium.TimeRange.From, consilium.TimeRange.Duration))
                .Returns(() => true);
            var consiliumService =
                new ConsiliumService(mockUnitOfWork.Object,mockService.Object);
            // Act
            var result = await consiliumService.ScheduleConsilium(consilium);
            Assert.NotNull(result);
        }
